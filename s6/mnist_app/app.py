...
def load_models():
    ...
    for name, path in model_paths.items():
        try:
            ...
            print(f"Model path: {os.path.abspath(path)}")
            model = Net().to(device)
...
def home():
    try:
        ...
        logger.info(f"Image paths being passed to template: {misclassified_images}")
...
def predict():
    try:
        ...
        with torch.no_grad():
            ...
            print(f"Raw model output range: [{output.min():.3f}, {output.max():.3f}]")
            ...
            for prob, idx in zip(top3_probs, top3_indices):
                print(f"Digit {idx.item()}: {prob.item():.2%}")
